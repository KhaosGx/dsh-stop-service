...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const StopCard = ({ t, info, onLocalStop, remoteStop }) => {
			...
			const onStop = async () => {
				// One-shot info fetch so the confirmation can name live activity:
				// running tasks escalate, idle sessions only count.
				...
				try {
					...
					if (data?.ok) {
						...
						if (running !== null && running > 0) {
							confirmText = t("confirmRunning").replace("{n}", String(running));
						} else if (total !== null && total > 0) {
							confirmText = t("confirmN").replace("{n}", String(total));
						}
					}
				} catch {
