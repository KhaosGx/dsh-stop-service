...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const InfoCard = ({ t, info, trends, windowId, setWindowId }) => {
			...
			const copyDiagnostics = async () => {
				...
				const lines = [
					...
					`Memory (RSS): ${info.memoryRssMb} MB · CPU: ${typeof info.cpuPercent === "number" ? `${info.cpuPercent}%` : "-"} · Sessions: ${typeof info.sessionCount === "number" ? info.sessionCount : "-"} (running: ${typeof info.runningSessions === "number" ? info.runningSessions : "-"})`,
				];
				try {
			...
			const sessionsValue = typeof info.sessionCount !== "number"
				? "-"
				: typeof info.runningSessions === "number" && info.runningSessions > 0
					? `${info.sessionCount}${t("runningSuffix").replace("{n}", String(info.runningSessions))}`
					: String(info.sessionCount);

			const rows = [
				[t("infoPid"), String(info.pid)],
				[t("infoStartedAt"), new Date(info.startedAt).toLocaleString()],
				[t("infoUptime"), formatUptime(info.uptimeMs, t)],
				[t("infoSessions"), sessionsValue],
				...
				const win = TREND_WINDOWS.find((w) => w.id === windowId) ?? TREND_WINDOWS[0];
				const memValues = (trends?.mem ?? []).slice(-win.samples).map((p) => p.v);
				const cpuValues = (trends?.cpu ?? []).slice(-win.samples).map((p) => p.v);
				const windowPicker = jsx("div", {
					key: "windows",
					style: { display: "flex", gap: 6, marginTop: 12 },
					children: TREND_WINDOWS.map((w) => jsx("button", {
						key: w.id,
						...
						style: {
							...
							borderRadius: 7,
							cursor: "pointer",
							border: w.id === windowId
								? "1px solid var(--dsw-accent, #4a90d9)"
								: "1px solid var(--dsw-border, rgba(128,128,128,0.35))",
							background: w.id === windowId ? "var(--dsw-accent, #4a90d9)" : "transparent",
							color: w.id === windowId ? "var(--dsw-danger-text, #fff)" : "inherit",
						},
						children: t("win_" + w.id)
					}, w.id))
				});
				return jsx("div", {
					...
					jsx(TrendChart, {
						key: "mem-chart",
						label: `${t("memTrend")} · ${t("win_" + win.id)}`,
						values: memValues
					}),
					jsx(TrendChart, {
						key: "cpu-chart",
						label: `${t("cpuTrend")} · ${t("win_" + win.id)}`,
						values: cpuValues
					}),
				typeof info.profile?.plugins?.length === "number" && info.profile.plugins.length > 0
					? jsx("details", {
						key: "components",
						...
						children: [
							jsx("summary", {
								key: "summary",
								style: { cursor: "pointer", opacity: 0.75, userSelect: "none" },
								children: `${t("compTitle")} (${info.profile.plugins.length})`
							}),
							...info.profile.plugins.map((p) => jsx("div", {
								key: p.name,
								style: rowStyle,
								children: [
									jsx("span", {
										key: "name",
										style: { ...labelStyle, wordBreak: "break-all", flexShrink: 1 },
										children: p.name
									}),
									jsx("span", {
										key: "ver",
										style: valueStyle,
										children: p.linked === true
											? `${p.version ?? "-"} ${t("linkedTag")}`
											: (p.version ?? "-")
									}),
								]
							}, p.name)),
		...
		/** Confirmation text for a stop request, escalated by live running tasks;
		 *  falls back to the generic line when /info is unreachable. */
		const stopConfirmText = async (t) => {
			try {
				const res = await fetch("/api/dsh-stop-service/info");
				const data = await res.json();
				if (data?.ok) {
					const running = typeof data.runningSessions === "number" ? data.runningSessions : null;
					const total = typeof data.sessionCount === "number" ? data.sessionCount : null;
					if (running !== null && running > 0) return t("confirmRunning").replace("{n}", String(running));
					if (total !== null && total > 0) return t("confirmN").replace("{n}", String(total));
				}
			} catch {
				// Info unavailable; generic confirmation below.
			}
			return t("confirm");
		};

		const StopCard = ({ t, info, onLocalStop, remoteStop }) => {
			const [state, setState] = react.useState("idle");
			const [drainSince, setDrainSince] = react.useState(null);
			const stopped = state === "stopped" || remoteStop === true;
			...
			// Draining: ride the panel's info snapshots and fire the normal stop
			// once nothing is running any more. The settings page must stay open.
			react.useEffect(() => {
				if (state !== "draining" || info === null) return;
				if (info.runningSessions === 0) {
					...
					void (async () => {
						try {
							...
							// The host dies right after replying; a network error
			...
			const onStop = async () => {
				// One-shot escalation: running tasks warn, idle sessions only count.
				if (!window.confirm(await stopConfirmText(t))) return;
				if (typeof onLocalStop === "function") onLocalStop();
				setState("stopping");
				try {
					await fetch("/api/dsh-stop-service/stop", { method: "POST" });
			...
			const onDrain = () => {
				const running = typeof info?.runningSessions === "number" ? info.runningSessions : null;
				const text = running !== null && running > 0
					? t("drainConfirmN").replace("{n}", String(running))
					: t("drainConfirm");
				if (!window.confirm(text)) return;
				setDrainSince(Date.now());
				setState("draining");
			};
			...
			if (state === "draining") {
				const running = typeof info?.runningSessions === "number" ? info.runningSessions : "-";
				const elapsedMs = drainSince === null ? 0 : Date.now() - drainSince;
				return jsx("div", {
					style: {
						...cardStyle,
						padding: "12px 16px",
					...
					children: [
						jsx("span", {
							key: "desc",
							style: { fontSize: 12.5, opacity: 0.85, lineHeight: 1.6 },
							children: t("draining").replace("{n}", String(running)).replace("{m}", formatUptime(elapsedMs, t))
						}),
						jsx("button", {
							key: "cancel",
							onClick: () => {
		 ...
		 * Each finding is actionable: copy the exact upgrade command, open the
		 * host npm page or the plugin releases. Offline-tolerant — the banner
		 * simply stays hidden.
		 */
		const UpdateBanner = ({ t, info }) => {
			...
			react.useEffect(() => {
				...
				(async () => {
					...
					if (hostLatest !== null && info.hostVersion && compareVersion(hostLatest, info.hostVersion) > 0) {
						found.push({ kind: "host", version: hostLatest, current: info.hostVersion });
					}
					if (pluginLatest !== null && info.pluginVersion && compareVersion(pluginLatest, info.pluginVersion) > 0) {
						found.push({ kind: "plugin", version: pluginLatest, current: info.pluginVersion });
					}
					setUpdates(found);
				})();
				return () => {
			...
			const commandFor = (u) => u.kind === "host"
				? `npm install -g @deepseek-ai/dsh@${u.version}`
				: `dsh plugin --profile web add dsh-stop-service@${u.version}`;
			const urlFor = (u) => u.kind === "host"
				? "https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"
				: "https://github.com/KhaosGx/dsh-stop-service/releases";
			const copy = async (u) => {
				try {
					await navigator.clipboard.writeText(commandFor(u));
					setCopiedKind(u.kind);
			...
			return jsx("div", {
				...
				children: updates.map((u) => jsx("div", {
					key: u.kind,
					...
					children: [
						jsx("span", {
							...
							children: t(u.kind === "host" ? "updHost" : "updPlugin").replaceAll("{v}", u.version).replaceAll("{c}", u.current)
						}),
						jsx("button", {
							key: "copy",
							onClick: () => copy(u),
							style: actionStyle,
							children: copiedKind === u.kind ? t("copied") : t("copyCmd")
						}),
						jsx("a", {
							key: "link",
							href: urlFor(u),
							...
							rel: "noreferrer",
							style: actionStyle,
							children: t(u.kind === "host" ? "linkNpm" : "linkReleases")
						}),
					]
				}, u.kind))
			});
		...
		const TREND_KEY = "dsh-stop-service:trends";
		const TREND_CAP = 720;
		const TREND_WINDOWS = [
			{ id: "3m", samples: 36 },
			{ id: "15m", samples: 180 },
		...
		/** Hydrate persisted trend samples; drops entries older than the cap window. */
		const loadStoredTrends = () => {
			try {
				const raw = JSON.parse(window.localStorage.getItem(TREND_KEY) || "null");
				if (raw === null || !Array.isArray(raw.mem) || !Array.isArray(raw.cpu)) return { mem: [], cpu: [] };
				const cutoff = Date.now() - TREND_CAP * 5000 - 60000;
				const clean = (arr) => arr.filter((p) => p && typeof p.t === "number" && typeof p.v === "number" && p.t >= cutoff);
				return { mem: clean(raw.mem), cpu: clean(raw.cpu) };
			} catch {
				return { mem: [], cpu: [] };
			}
		};

		/** Best-effort persist; storage failures keep trends in memory only. */
		const storeTrends = (mem, cpu) => {
			try {
				window.localStorage.setItem(TREND_KEY, JSON.stringify({ mem, cpu }));
			} catch {
		...
		const StopServiceTab = ({ t }) => {
			...
			const [trends, setTrends] = react.useState(loadStoredTrends);
			const [windowId, setWindowId] = react.useState("3m");
			...
			const [remoteStop, setRemoteStop] = react.useState(false);
			const channelRef = react.useRef(null);

			// Coordinate stop intent across tabs: a stop fired in one tab must not
			// leave the others silently showing stale data.
			react.useEffect(() => {
				let ch = null;
				try {
					ch = new BroadcastChannel("dsh-stop-service");
				} catch {
					return;
				}
				channelRef.current = ch;
				ch.onmessage = (ev) => {
					if (ev?.data?.type === "stopping") {
						setRemoteStop(true);
						setLocalStop(true);
					}
				};
			...
			const announceStop = () => {
				setLocalStop(true);
				try {
					channelRef.current?.postMessage({ type: "stopping" });
				} catch {
					// No channel; other tabs fall back to the stale banner.
				}
			};
				...
				const load = async () => {
					try {
						...
						if (alive && res.ok && data?.ok) {
							...
							setTrends((prev) => {
								const now = Date.now();
								const next = { mem: prev.mem, cpu: prev.cpu };
								if (typeof data.memoryRssMb === "number") {
									next.mem = [...prev.mem, { t: now, v: data.memoryRssMb }].slice(-TREND_CAP);
								}
								if (typeof data.cpuPercent === "number") {
									next.cpu = [...prev.cpu, { t: now, v: data.cpuPercent }].slice(-TREND_CAP);
								}
								storeTrends(next.mem, next.cpu);
								return next;
							});
						} else if (alive) {
			...
			return jsx("div", {
				...
				children: [
					...
					jsx(StopCard, { key: "stop", t: t, info: info, remoteStop: remoteStop, onLocalStop: announceStop }),
					jsx(InfoCard, {
						key: "info",
						t: t,
						info: info,
		...
		/**
		 * Settings-shell header action: the same confirmed stop without
		 * navigating to the Service section. Broadcasts to sibling tabs and
		 * watches for the restarted host to reload, mirroring the section card.
		 */
		const QuickStopAction = ({ t }) => {
			...
			const onClick = async () => {
				...
				setFired(true);
				try {
					const ch = new BroadcastChannel("dsh-stop-service");
					ch.postMessage({ type: "stopping" });
					ch.close();
				} catch {
					// Other tabs fall back to the stale banner.
				}
				try {
					await fetch("/api/dsh-stop-service/stop", { method: "POST" });
				} catch {
			...
			return jsx("button", {
				...
				children: fired ? t("stopping") : t("stop")
			});
		};
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, {
				zh: {
					...
					linkReleases: "发行说明",
					stop: "终止服务",
					quickStopTitle: "优雅终止 DSH 宿主进程（SIGTERM）",
					stopping: "正在停止…",
					...
					confirmRunning: "⚠ 有 {n} 个会话的任务正在运行，现在终止将中断它们（已保存的会话不受影响，未发送的输入将丢失）。确定要终止服务吗？",
					runningSuffix: "（{n} 个运行中）",
					drainStop: "任务结束后停止",
					drainConfirm: "所有任务结束后将自动终止服务（需保持本设置页打开）。确定吗？",
					drainConfirmN: "{n} 个任务结束后将自动终止服务（需保持本设置页打开）。确定吗？",
					draining: "等待 {n} 个运行中任务结束（已等 {m}）… 全部结束后服务将自动终止",
					drainCancel: "取消等待",
				...
				en: {
					...
					win_15m: "15 min",
					win_1h: "1 hour",
					compTitle: "Loaded plugins",
					linkedTag: "(local link)",
					infoNode: "Node",
					...
					stop: "Stop Service",
					quickStopTitle: "Gracefully stop the DSH host process (SIGTERM)",
					stopping: "Stopping…",
					...
					confirmRunning: "⚠ {n} sessions still have tasks running — stopping now interrupts them (saved sessions are unaffected, unsent input is lost). Stop anyway?",
					runningSuffix: " ({n} running)",
					drainStop: "Stop After Tasks Finish",
					drainConfirm: "Once every running task finishes, the service stops automatically (keep this settings page open). Proceed?",
					drainConfirmN: "The service stops automatically once these {n} running task(s) finish (keep this settings page open). Proceed?",
					draining: "Waiting for {n} running task(s) ({m} elapsed)… the service stops automatically when they finish",
					drainCancel: "Cancel wait",
			...
			ctx.effect(() => ctx.slots.inject("settings.action", () => ctx.slots.register({
				...
				locale: NS,
				inject: () => ({ t })
			}, QuickStopAction)), "stop-service: settings action");
		}
