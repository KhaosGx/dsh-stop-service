...
/**
 * Profile composition info: bundles list and plugin dependencies with their
 * installed versions (and link: installs flagged) from the web profile's
 * package.json + node_modules. Read-only; degrades to null.
 */
function profileInfo(config) {
  try {
    const webDir = join(dshHomeOf(config), "profiles", "web");
    const manifest = JSON.parse(readFileSync(join(webDir, "package.json"), "utf8"));
    const deps = manifest?.dependencies ?? {};
    const plugins = Object.keys(deps).map((dep) => {
      const range = String(deps[dep] ?? "");
      let version = null;
      try {
        const installed = JSON.parse(readFileSync(join(webDir, "node_modules", dep, "package.json"), "utf8"));
        if (typeof installed?.version === "string") version = installed.version;
      } catch {
        // Not installed / unreadable; report without a version.
      }
      return { name: dep, version, linked: range.startsWith("link:") };
    });
    return {
      name: "web",
...
function makeRoutes(ctx, config) {
  /**
   * User-facing session figures from the agent registry: top-level sessions
   * (sub-agents excluded — the count a human sees in the sidebar) and how
   * many of them are mid-run. Falls back to the raw session store (count
   * only, includes sub-agents); nulls when neither registry answers.
   */
  const sessionFigures = () => {
    try {
      if (typeof ctx.agents?.roots === "function") {
        const roots = ctx.agents.roots();
        return {
          sessionCount: roots.length,
          runningSessions: roots.filter((agent) => agent?.status === "running").length,
        };
      }
    } catch {
      // Registry unavailable; fall through to the session store.
    }
    try {
      return {
        sessionCount: typeof ctx.sessions?.list === "function" ? ctx.sessions.list().length : null,
        runningSessions: null,
      };
    } catch {
      return { sessionCount: null, runningSessions: null };
    }
  };
  return [
    {
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        writeJson(res, 200, {
          ...
          sessionCount: figures.sessionCount,
          runningSessions: figures.runningSessions,
